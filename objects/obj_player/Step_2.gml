...
switch (sprite_index)
{
	...
	case spr_player_row:
		// Check if 'm' is pressed and either left or right arrow keys are held down
		if (keyboard_check(vk_left) || keyboard_check(vk_right) || keyboard_check(ord("A")) || keyboard_check(ord("D")) || external_row_right_press || external_row_left_press) {
			// Start rowing animation
			// In that case we change its sprite to the idle sprite.
			sprite_index = spr_player_row;

			// We also reset the animation speed to 1, as it was set to 0 at the end of the fall animation.
			image_speed = 1;
		} else {
			// Stop rowing animation or switch to idle animation if neither left nor right is pressed
			image_speed = 0; // Stops the animation
			sprite_index = spr_player_row;
		}
		 external_row_right_press = false;
		 external_row_left_press = false;
		break;
